  ...
  return (
    <button
      ...
      style={{
        ...
        height: 26,
        borderRadius: 13,
        background: on ? 'linear-gradient(135deg, #0ea5e9, #0284c7)' : '#1e2838',
...
export default function SettingsScreen({ onNavigate }: Props) {
  ...
  return (
    <div className="absolute inset-0 flex flex-col" style={{ background: '#080c14' }}>
      {/* Header */}
      <div className="flex-shrink-0 pt-14 px-5 pb-4" style={{ background: 'linear-gradient(180deg, #0d1420 0%, #080c14 100%)' }}>
      ...
      <div className="flex-1 overflow-y-auto pb-20 px-5" style={{ scrollbarWidth: 'none' }}>
        ...
        <div
          ...
          style={{ background: 'linear-gradient(135deg, rgba(14,165,233,0.1), rgba(2,132,199,0.05))', border: '1px solid rgba(14,165,233,0.2)' }}
        >
          <div className="relative flex-shrink-0">
            <div className="flex items-center justify-center rounded-2xl font-black text-2xl"
              style={{ width: 58, height: 58, background: 'linear-gradient(135deg, #0ea5e9, #0284c7)', color: '#fff', boxShadow: '0 0 20px rgba(14,165,233,0.3)' }}>
              M
            </div>
            <div className="absolute -bottom-1 -right-1 rounded-full flex items-center justify-center"
              style={{ width: 18, height: 18, background: '#22c55e', border: '2px solid #080c14' }}>
        ...
        <SettingGroup title="General">
          <SettingRow icon={Globe} iconColor="#0ea5e9" label="Language" value="English" showChevron />
        ...
        <SettingGroup title="Playback">
            ...
            <div className="flex gap-2">
              {(['exo', 'vlc'] as const).map((p) => (
                <button key={p} onClick={() => setLivePlayer(p)}
                  ...
                  style={{
                    background: livePlayer === p ? 'rgba(14,165,233,0.15)' : '#0d1117',
                    border: `1px solid ${livePlayer === p ? 'rgba(14,165,233,0.4)' : '#1a2030'}`,
            ...
            <div className="flex gap-2">
              {(['exo', 'vlc'] as const).map((p) => (
                <button key={p} onClick={() => setMoviePlayer(p)}
                  className="flex-1 py-2 rounded-xl text-xs font-bold uppercase tracking-wide"
                  style={{
                    background: moviePlayer === p ? 'rgba(14,165,233,0.15)' : '#0d1117',
                    border: `1px solid ${moviePlayer === p ? 'rgba(14,165,233,0.4)' : '#1a2030'}`,
          ...
          <SettingRow icon={Tv2} iconColor="#0ea5e9" label="Hardware Acceleration">
        ...
        <SettingGroup title="Data & Sync">
          ...
          <SettingRow icon={RefreshCw} iconColor="#0ea5e9" label="Refresh Playlists" showChevron>
            <button
        ...
        {/* App info card */}
        <div className="rounded-2xl overflow-hidden mb-4" style={{ border: '1px solid #1e2838' }}>
          <div className="p-4" style={{ background: '#0d1117' }}>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-semibold" style={{ color: '#0ea5e9' }}>my.maxxplayer.com</span>
            </div>
      ...
      {showLogout && (
        ...
        >
          <div className="w-full rounded-t-3xl p-6" style={{ background: '#12161e', border: '1px solid #1e2838' }}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-black" style={{ color: '#f8fafc' }}>Logout</h3>
              <button onClick={() => setShowLogout(false)} className="flex items-center justify-center rounded-full" style={{ width: 28, height: 28, background: '#1e2838' }}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowLogout(false)} className="flex-1 py-3.5 rounded-2xl font-semibold text-sm"
                style={{ background: '#1e2838', color: '#94a3b8' }}>
                Cancel
              </button>
              <button onClick={() => onNavigate('login')} className="flex-1 py-3.5 rounded-2xl font-semibold text-sm"
...
function SettingGroup({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: '1px solid #1e2838' }}>
        {children}
      </div>
    </div>
  );
...
function SettingRow({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3.5" style={{ background: '#12161e', borderTop: '1px solid #1a2030' }}>
