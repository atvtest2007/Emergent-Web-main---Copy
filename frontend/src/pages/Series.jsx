...
export default function Series() {
    ...
    return (
        <div className="px-6 lg:px-12 py-8 pt-12" data-testid="series-catalog-page">
            <div className="mb-8">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6 sticky top-0 py-3 z-20 bg-[#050505]/80 backdrop-blur-md">
                <div className="relative flex-1 max-w-md">
                    ...
                    <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search series..." className="pl-9 bg-[#0a0a0a] border-white/10 text-zinc-100 focus:border-[#E50914]" data-testid="series-search" />
                </div>
                <div className="flex flex-wrap gap-2 overflow-x-auto rail-scroll">
                    <button onClick={() => setCat("all")} className={`px-3 py-1.5 rounded-md text-xs font-bold tracking-wider uppercase whitespace-nowrap transition ${cat === "all" ? "bg-[#E50914] text-white" : "bg-white/5 text-zinc-300 hover:bg-white/10"}`} data-testid="series-cat-all">
                        All ({items.length})
                    </button>
                    {cats.map((c) => (
                        <button key={c.category_id} onClick={() => setCat(c.category_id)} className={`px-3 py-1.5 rounded-md text-xs font-bold tracking-wider uppercase whitespace-nowrap transition ${cat === c.category_id ? "bg-[#E50914] text-white" : "bg-white/5 text-zinc-300 hover:bg-white/10"}`}>
                            {c.category_name}
                        </button>
                    ))}
                </div>
