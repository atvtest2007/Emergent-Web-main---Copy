...
export default function HistoryPage() {
    ...
    return (
        <div className="px-6 lg:px-12 py-8 pt-12" data-testid="history-page">
            <div className="mb-8">
            ...
            {loading ? (
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {items.map((it) => {
                        ...
                        return (
                            <div key={`${it.content_type}-${it.content_id}`} className="group relative rounded-md overflow-hidden bg-[#121212] border border-white/5 hover:border-[#E50914]/40 transition" data-testid={`history-${it.content_id}`}>
                                <Link to={watchTo} className="block">
                                    <div className="relative aspect-video bg-black overflow-hidden">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                                            <div className="w-14 h-14 rounded-full bg-[#E50914] flex items-center justify-center">
                                                <Play className="w-6 h-6 fill-white ml-0.5" />
                                            </div>
                                        </div>
                                        <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/15">
                                            <div className="h-full bg-[#E50914]" style={{ width: `${Math.min(100, (it.progress || 0) * 100)}%` }} />
                                        </div>
                                    </div>
                                    <div className="p-3">
                                ...
                                <button onClick={() => remove(it)} className="absolute top-2 right-2 w-8 h-8 rounded-full glass border border-white/10 flex items-center justify-center hover:bg-[#E50914]/20" data-testid={`remove-history-${it.content_id}`}>
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        );
