...
export default function FavoritesPage() {
    ...
    return (
        <div className="px-6 lg:px-12 py-8 pt-12" data-testid="favorites-page">
            <div className="mb-8">
                    ...
                    {movies.length > 0 && (
                        <section className="mb-10">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                {movies.map((m) => <PosterCard key={m.id} item={{ ...m.content_data, stream_id: m.content_id }} type="movie" />)}
                            </div>
                        </section>
                    )}
                    {series.length > 0 && (
                        <section className="mb-10">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                {series.map((s) => <PosterCard key={s.id} item={{ ...s.content_data, series_id: s.content_id }} type="series" />)}
                            </div>
                        </section>
                    )}
                    {live.length > 0 && (
