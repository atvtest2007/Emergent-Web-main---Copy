...
export default function TVSplashScreen() {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center"
      ...
      <div className="absolute rounded-full pointer-events-none" style={{
        width: 300, height: 300, border: '1px solid rgba(229,9,20,0.08)',
        top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
      }} />
      <div className="absolute rounded-full pointer-events-none" style={{
        width: 420, height: 420, border: '1px solid rgba(229,9,20,0.05)',
        top: '50%', left: '50%', transform: 'translate(-50%, -50%)',
      }} />
        ...
        <div className="flex items-center gap-3 mt-2">
          {['4K HDR', 'Dolby Atmos', 'Multi-Platform', 'Xtream / M3U'].map((feat) => (
            <div key={feat} className="px-4 py-1.5 rounded-full text-sm font-semibold"
              style={{ background: 'rgba(229,9,20,0.1)', border: '1px solid rgba(229,9,20,0.2)', color: '#38bdf8' }}>
              {feat}
            </div>
          ))}
        </div>
      ...
      <p className="absolute bottom-6 text-sm" style={{ color: '#2A2A2A' }}>v1.0.0 · my.maxxplayer.com</p>
    </div>
  );
}
