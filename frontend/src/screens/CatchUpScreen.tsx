    ...
    <div className="absolute inset-0 flex flex-col" style={{ background: '#0a0a0f' }}>
      <div className="flex-shrink-0 pt-14 px-5 pb-3" style={{ background: 'linear-gradient(180deg, #0d1117 0%, #0a0a0f 100%)' }}>
        ...
        <div className="flex items-center gap-2">
          <button className="flex items-center justify-center rounded-xl flex-shrink-0" style={{ width: 32, height: 32, background: '#1A1A1A', border: '1px solid #2A2A2A' }}>
      ...
      <div className="flex-1 overflow-y-auto pb-20 px-5 flex flex-col gap-4">
        {catchUpData.map((item) => (
          <div key={item.channel} className="rounded-2xl overflow-hidden" style={{ background: '#1A1A1A', border: '1px solid #2A2A2A' }}>
            <div className="flex items-center gap-3 p-3 border-b" style={{ borderColor: '#1a2030' }}>
              ...
              <span className="text-xs px-2 py-1 rounded-full font-semibold" style={{ background: 'rgba(229,9,20,0.1)', color: '#38bdf8', border: '1px solid rgba(229,9,20,0.2)' }}>
            ...
            <div className="p-3">
              <div className="flex gap-2 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                {item.slots.map((slot, idx) => (
                  <button key={slot} onClick={() => navigate('/watch/catchup/mock')}
                    className="flex-shrink-0 flex flex-col items-center gap-1.5 px-3 py-2.5 rounded-xl"
                    style={{
                      background: idx === 2 ? 'rgba(229,9,20,0.15)' : '#0d1117',
